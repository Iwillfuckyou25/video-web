...
const INDEXNOW_KEY = '296ee509811b2a4476de1811a0b486c9';
...
const notifyIndexNow = async paths => {
  const urlList = [...new Set(paths)].map(value => new URL(value, `${SITE_URL}/`).href).filter(value => new URL(value).host === new URL(SITE_URL).host);
  if (!urlList.length || !IS_PRODUCTION) return;
  const response = await fetch('https://api.indexnow.org/indexnow', { method: 'POST', headers: { 'Content-Type': 'application/json; charset=utf-8' }, body: JSON.stringify({ host: new URL(SITE_URL).host, key: INDEXNOW_KEY, keyLocation: `${SITE_URL}/${INDEXNOW_KEY}.txt`, urlList }) });
...
const processVideoVariants = async ({ videoId, inputPath: suppliedInputPath, id }) => {
  ...
  try {
    ...
    if (targetStatus === 'published') notifyIndexNow([`/watch/${videoId}`, '/', '/latest', '/sitemap.xml', '/video-sitemap.xml', '/feed.xml']).catch(error => console.error('IndexNow notification failed:', error.message));
...
app.get('/sitemap.xml', async (_req, res, next) => { try { const videos = await Video.find({ status: 'published', processingStatus: 'ready' }).select('_id updatedAt').lean(); const fixed = Object.keys(staticSeo); const urls = [...fixed.map(url => ({ url, lastmod: STATIC_LAST_MODIFIED })), ...videos.map(video => ({ url: `/watch/${video._id}`, lastmod: new Date(video.updatedAt || STATIC_LAST_MODIFIED).toISOString() }))]; res.set('Cache-Control', 'public, max-age=3600').type('xml').send(`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls.map(item => `<url><loc>${xmlEscape(absoluteUrl(item.url))}</loc><lastmod>${item.lastmod}</lastmod></url>`).join('')}</urlset>`); } catch (error) { next(error); } });
app.get('/video-sitemap.xml', async (_req, res, next) => { try { const videos = await Video.find({ status: 'published', processingStatus: 'ready' }).sort({ uploadDate: -1 }).limit(1000).lean(); res.set('Cache-Control', 'public, max-age=3600').type('xml').send(`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:video="http://www.google.com/schemas/sitemap-video/1.1">${videos.map(video => `<url><loc>${xmlEscape(absoluteUrl(`/watch/${video._id}`))}</loc><video:video><video:thumbnail_loc>${xmlEscape(absoluteUrl(`/media/${video._id}/thumbnail`))}</video:thumbnail_loc><video:title>${xmlEscape(video.title)}</video:title><video:description>${xmlEscape(clean(video.description, 1800) || video.title)}</video:description><video:content_loc>${xmlEscape(absoluteUrl(`/media/${video._id}/video`))}</video:content_loc><video:publication_date>${new Date(video.uploadDate || video.createdAt).toISOString()}</video:publication_date><video:family_friendly>no</video:family_friendly></video:video></url>`).join('')}</urlset>`); } catch (error) { next(error); } });
app.get('/feed.xml', async (_req, res, next) => { try { const videos = await Video.find({ status: 'published', processingStatus: 'ready' }).sort({ uploadDate: -1 }).limit(50).select('_id title description category uploadDate createdAt').lean(); res.set('Cache-Control', 'public, max-age=1800').type('application/rss+xml').send(`<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel><title>S3X Video latest uploads</title><link>${xmlEscape(SITE_URL)}</link><description>Recently added videos on S3X Video.</description><language>en</language><lastBuildDate>${new Date().toUTCString()}</lastBuildDate>${videos.map(video => { const link = absoluteUrl(`/watch/${video._id}`); return `<item><title>${xmlEscape(video.title)}</title><link>${xmlEscape(link)}</link><guid isPermaLink="true">${xmlEscape(link)}</guid><description>${xmlEscape(clean(video.description, 500) || video.title)}</description><category>${xmlEscape(video.category)}</category><pubDate>${new Date(video.uploadDate || video.createdAt).toUTCString()}</pubDate></item>`; }).join('')}</channel></rss>`); } catch (error) { next(error); } });
